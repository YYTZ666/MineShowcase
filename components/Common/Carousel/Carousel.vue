...
import {
    useRafFn,
    useEventListener as useVueUseEventListener,
    useResizeObserver as useVueUseResizeObserver,
    useTransition,
} from '@vueuse/core'
...
export interface Props {
    ...
    pauseOnMouseEnter?: boolean // 鼠标移入时是否暂停自动轮播
    effect?: 'slide' | 'fade' // 轮播图切换过渡效果
    ...
    spinProps?: object // 图片加载中样式，Spin 组件属性配置
    fadeDuration?: number // 渐变动画持续时长，单位 ms，仅在 effect 为 'fade' 时生效
    fadeFunction?: string // 渐变动画函数，仅在 effect 为 'fade' 时生效
    slideDuration?: number // 滑动动画持续时长，单位 ms，仅在 effect 为 'slide' 时生效
    slideFunction?: string | number[] // 滑动动画函数，仅在 effect 为 'slide' 时生效
}
...
const slideTimer = ref<ReturnType<typeof useRafFn>>() // 轮播切换定时器
const stopCarousel = ref(false) // 鼠标悬浮时，停止切换标志
const switchPrevent = ref(false) // 在滑动切换过程中，禁用其他所有切换操作
const carouselRef = ref<HTMLElement>() // carousel DOM 引用
...
const carouselStyle = computed(() => {
    if (props.effect === 'slide') {
        return {
            transform:
                (verticalSlide.value ? 'translateY' : 'translateX') +
                `(${-offset.value}px)`,
        }
    } else {
        return {}
...
useVueUseEventListener(document, 'visibilitychange', visibilityChange)
useVueUseResizeObserver(carouselRef, () => {
    getImageSize()
    initCarousel()
})

function initCarousel(): void {
    slideTimer.value?.pause()
    switchPrevent.value = false
    if (props.effect === 'slide') {
        offset.value = (activeSwitcher.value - 1) * moveDistance.value
    }
    onStart()
}
...
function autoSlide() {
    if (!stopCarousel.value) {
        slideTimer.value?.pause()
        const fpsLimit = 1000 / props.interval
        slideTimer.value = useRafFn(
            () => {
                switchPrevent.value = true // 禁用导航切换
                if (props.effect === 'slide') {
                    const target =
                        (offset.value %
                            (imageAmount.value * moveDistance.value)) +
                        moveDistance.value
                    moveLeft(target)
                    activeSwitcher.value =
                        (activeSwitcher.value % imageAmount.value) + 1
                } else {
                    // fade
                    moveFade('left')
                }
...
function onLeftArrow(): void {
    if (!switchPrevent.value) {
        ...
        if (props.effect === 'slide') {
            const target =
                ((activeSwitcher.value + imageAmount.value - 2) %
                    imageAmount.value) *
                moveDistance.value
            moveRight(target)
            activeSwitcher.value =
                activeSwitcher.value - 1 > 0
                    ? activeSwitcher.value - 1
                    : imageAmount.value
        } else {
            // fade
            moveFade('right')
...
function onRightArrow(): void {
    if (!switchPrevent.value) {
        ...
        slideTimer.value?.pause()
        if (props.effect === 'slide') {
            const target = activeSwitcher.value * moveDistance.value
            moveLeft(target)
            activeSwitcher.value =
                (activeSwitcher.value % imageAmount.value) + 1
        } else {
...
function moveEffect(): void {
    if (baseNumber.value) {
        offset.value =
            originNumber.value + distance.value * cubicBezierNumber.value
    } else {
        offset.value =
            originNumber.value + distance.value * (1 - cubicBezierNumber.value)
    }
}

function moveLeftEffect(): void {
    if (offset.value >= originNumber.value + distance.value) {
        switchPrevent.value = false
        if (props.autoplay) {
            autoSlide() // 自动间隔切换下一张
        }
...
function moveLeft(target: number): void {
    if (offset.value === imageAmount.value * moveDistance.value) {
        // 最后一张时，重置left
        offset.value = 0
    }
    toggleNumber(target)
...
function moveRight(target: number): void {
    if (offset.value === 0) {
        // 第一张时，重置left
        offset.value = imageAmount.value * moveDistance.value
    }
    toggleNumber(target)
    requestAnimationFrame(moveRightEffect)
}
...
function onSwitch(n: number): void {
    if (!switchPrevent.value && activeSwitcher.value !== n) {
        ...
        if (n < activeSwitcher.value) {
            // 往右滑动
            if (props.effect === 'slide') {
                const target = (n - 1) * moveDistance.value
                moveRight(target)
    ...
    >
        <div class="carousel-flex-wrap" :style="carouselStyle">
            <div
                ...
                :class="{
                    'image-fade-active':
                        effect === 'fade' && activeSwitcher === index + 1,
                }"
            ...
            <div
                class="image-wrap"
                @click="clickImage(images[0])"
                v-if="imageAmount && effect === 'slide'"
            >
                <a-spin
                    :spinning="!complete[0]"
                    indicator="dynamic-circle"
        ...
        >
            <div
                ...
                :style="[
                    ...
                    activeSwitcher === n
                        ? {
                              backgroundColor: dotActiveColorComputed,
                              ...dotActiveStyle,
                          }
                        : {},
                ]"
                ...
                @mouseenter="
                    dotsTrigger === 'hover' ? onMouseEnter(n) : () => false
                "
                @keydown.prevent="onKeyboard"
            ></div>
        </div>
