      ...
      <div class="stats-panel">
        <div class="stat-group">
          <div class="stat-item" v-for="(item, index) in stats" :key="index">
            <div class="stat-value" :id="item.id">{{ item.value }}</div>
            <div class="stat-label">{{ item.label }}</div>
          </div>
        </div>
      </div>
      
      <div class="info-card" :class="{ active: isCardActive, entering: isEntering }">
        <div class="card-glow"></div>
        <div class="card-content">
          <div class="brand">
            <div class="logo-container">
              <img :src="logoUrl" alt="MSCPO" class="logo">
            </div>
          </div>
          ...
          <div class="feature-showcase">
            <div class="feature-card">
              <div 
                ...
                :class="{ current: currentFeatureIndex === index }"
                :data-index="index"
          ...
          <div class="actions">
            <a 
              href="https://mscpo.crashvibe.cn/serverlist" 
              class="action-button btn-primary"
              @click.prevent="navigateTo('https://mscpo.crashvibe.cn/serverlist')"
            ...
            <a 
              href="https://mscpo.crashvibe.cn/about" 
              class="action-button btn-secondary"
              @click.prevent="navigateTo('https://mscpo.crashvibe.cn/about')"
              @click="createRipple"
            >
...
export default {
  ...
  data() {
    return {
      isCardActive: false,
      isEntering: false,
      logoUrl: 'https://free.boltp.com/2025/05/09/681e01399f508.webp',
      currentFeatureIndex: 0,
      autoPlayInterval: null,
      stats: [
        { id: 'serverCount', value: 0, label: '服务器', target: 0 },
        { id: 'onlinePlayers', value: 0, label: '在线玩家', target: 0 },
        { id: 'totalViews', value: 0, label: '访问量', target: 0 }
      ],
      features: [
  ...
  methods: {
    ...
    handleFeatureMouseMove(event, index) {
      const content = event.currentTarget;
      const rect = content.getBoundingClientRect();
      const x = ((event.clientX - rect.left) / rect.width) * 100;
      const y = ((event.clientY - rect.top) / rect.height) * 100;
      
      const highlight = content.querySelector('.feature-highlight');
      if (highlight) {
    ...
    async fetchStats() {
      try {
        ...
        // 更新统计数据
        this.stats[1].target = playerData.total_players || 0;
        this.stats[0].target = 15; // 服务器数量
        this.stats[2].target = 3500; // 访问量
        ...
        // 设置自动刷新
        this.statsRefreshInterval = setInterval(() => this.fetchStats(), 30000);
      } catch (error) {
    ...
    async fetchPlayerCount() {
      ...
      try {
        ...
        const data = { total_players: Math.floor(Math.random() * 100) + 50 };
    ...
    animateNumber(stat) {
      ...
      const updateDisplay = (currentTime) => {
        ...
        stat.value = Math.round(start + (end - start) * progress);
